...
let tagged_item_list_to_string ?(style:[style|`ComputerWrapped of string|`LanguageWrapped of string]=`Normal)  items =
    ...
    | `ComputerWrapped title -> 
            ...
            (fun x -> [`CVcomputer {category=(`String title); programs=(`String (x |> List.map cvitem_to_latex |> List.map latex_to_string |> List.map String.trim |> String.concat " · "))}]))
...
let get_latex_info ?(include_photo=false) (items: (Base_types.tags * Base_types.cvitem) list ) =
    ...
    let latex_list = match info.website with None -> latex_list
        | Some (link, text) -> (`Command ("extrainfo", [ `Command ("href", [`String link; `String text] ) ]
        )::latex_list)
    in 
    `Concat latex_list
        |> latex_to_string
